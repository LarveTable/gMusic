...
class MusicCog(commands.Cog):
    ...
    async def youtube(self, interaction: discord.Interaction, song : str):
        ...
        if (interaction.guild.voice_client and interaction.guild.voice_client.is_playing()):
            embed = discord.Embed(
                title='I am already playing music. '+os.getenv('ALREADY_PLAYING'),
                description='Please stop the current music before playing a new one. Period.',
            ...
            try:
                ...
                if (yt != None):
                    ...
                    if voice_client.is_connected():
                        voice_client.play(discord.FFmpegPCMAudio(yt['path']), after=lambda e: self.bot.loop.create_task(finished_playing(message)))
                        embed = discord.Embed(
...
async def finished_playing(message):
    embed = discord.Embed(
